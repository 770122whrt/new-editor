...
globs: packages/viewer/**,packages/editor/**,apps/editor/**
alwaysApply: false
---
...
|---|---|---|---|
| `SCENE_LAYER` | `0` | `@pascal-app/viewer` | Default Three.js layer for regular scene geometry |
| `EDITOR_LAYER` | `1` | `@pascal-app/editor` | Editor-only helpers: grid, tool previews, cursor meshes, snap guides |
| `ZONE_LAYER` | `2` | `@pascal-app/viewer` | Zone floor fills and wall borders composited in a separate post-processing pass |

Import constants from their owning packages:

```ts
// In viewer code
import { SCENE_LAYER, ZONE_LAYER } from '@pascal-app/viewer'

// In editor package code
...
The editor camera enables `EDITOR_LAYER` so tools and helpers are visible during editing. Thumbnail/export flows disable `EDITOR_LAYER` so captures show clean geometry without snap lines, cursors, or tool previews.
...
- **`EDITOR_LAYER` belongs in `@pascal-app/editor`.** The viewer must never import it; editor behavior is injected via props/children.
- **Zone meshes must set `layers={ZONE_LAYER}`** so they are picked up by `zonePass` and excluded from scene depth buffers.
- **Editor helper meshes must set `layers={EDITOR_LAYER}`** so they are invisible to thumbnail/export cameras and viewer render passes that intentionally exclude editor helpers.
- **Do not add new layers without updating this rule** and the post-processing/export pipeline accordingly.
